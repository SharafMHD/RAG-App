...
cd ..
cp rag-app/alembi.example.ini rag-app/alembic.ini
```
...
docker compose up -d pgvector qdrant postgres_exporter
# Wait for databases to be healthy
sleep 30
# Start the application services
docker compose up fastapi nginx prometheus grafana node_exporter --build -d
```
...
- FastAPI health/readiness: http://localhost:8000/api/v1/health
...
FastAPI is configured to expose Prometheus metrics at the `/metrics` endpoint. The legacy `/metrics24` endpoint is also kept for the bundled Prometheus configuration. These metrics include:
...
The application now validates configuration at startup and includes default security middleware:

- common security headers
- CORS allow-list via `CORS_ALLOWED_ORIGINS`
- optional API-key enforcement with `REQUIRE_API_KEY=true` and `API_KEY=...`
- simple per-client rate limiting via `RATE_LIMIT_*`
- trusted host allow-list via `TRUSTED_HOSTS`

For production, do not use wildcard CORS/trusted hosts and provide secrets from a secret manager or deployment environment rather than committing them to env files.
